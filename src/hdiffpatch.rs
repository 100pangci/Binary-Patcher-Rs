...
pub fn run_hdiffz_mem(
    ...
    crate::utils::ensure_parent_dir(patch_file)
        .map_err(|e| ffi::PatchError { code: -1, message: e.to_string() })?;
    ...
    std::fs::write(patch_file, &patch_data)
        .map_err(|e| ffi::PatchError { code: -1, message: format!("写入补丁文件失败 {}: {e}", patch_file.display()) })?;
    Ok(thread_count)
}
...
pub fn run_hdiffz(
    ...
    let mem_result = (|| -> Result<(), ffi::PatchError> {
        ...
        let new_data = std::fs::read(new_file)
            .map_err(|e| ffi::PatchError { code: -1, message: format!("读取新文件失败 {}: {e}", new_file.display()) })?;
        let patch_data = ffi::create_patch(&old_data, &new_data, thread_count, use_compression, fast_format)?;
        std::fs::write(patch_file, &patch_data)
            .map_err(|e| ffi::PatchError { code: -1, message: format!("写入补丁文件失败 {}: {e}", patch_file.display()) })?;
        Ok(())
